 ...
 * jcache probe folds offsetof(JitEnv, jcache) into an ADD imm12 that it masks
 * to 12 bits — a struct that outgrew the range would not fail to compile, it
 * would silently address the wrong field. Bound them here so adding a member
 * ahead of jcache, or widening CPU, is a build error on every host rather
 * than a miscompile on one. */
