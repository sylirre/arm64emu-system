 ...
 * leak host OFC. Both are far outside anything a guest exercises. NaN
 * *results* used to be a third such corner and are no longer: both which NaN
 * an operation returns and what a generated one looks like are decided here
 * rather than inherited from the host — see the FPProcessNaNs / DefaultNaN
 * block below. FPCR.DN (Default-NaN mode, which would make even a propagated
 * NaN come back as the default) is still not modelled. */
...
static double fop_d_raw(unsigned op, double n, double m, double d) {
    switch (op) {
        ...
        case FOP_RSQRTS: {                    /* FPRSqrtStepFused: 0*inf -> 1.5 */
             ...
             * to inf where the halved value is still representable) and in
             * the subnormal range. Halving is exact only above the bottom
             * exponent, so pick the operand it cannot clip (checked in the
             * integer domain — an FP probe would raise flags the guest never
             * asked for); when both sit at the very bottom the product is
             * far below 3's significance and the old shape is exact, with
             * the right IXC. */
