...
- **Host FP semantics** (all engines equally, `docs/todo/TODO_OPCODES.md` §6):
  ...
  UFC boundary).
- **Debug facilities force the interpreter** (`-d`, trace, watchpoints,
...
2. **Uncounted NOPs** (`frontend.c`, `PD_NOP`). The frontend consumed
   ...
   slow, and `-maxinsn` overshot (a 600-NOP image blew ~200 insns past the
...
boundary — and any `-maxinsn` stop, via the interpret-tail — is exact; the
